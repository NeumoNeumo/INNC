...
CatBack::CatBack(
    TensorImpl *this_tf,
    const std::vector<std::shared_ptr<INNC::TensorImpl>> &input_tfs, const size_t dim)
...
void CatBack::step_back() {
  ...
  for (size_t i = 0; i < input_tfs.size(); i++){
    auto ret = INNC::TensorImpl::create(
        this_tf->dtype, std::make_unique<StridedLayout>(input_tfs[i]->view->sizes, dynamic_cast<StridedLayout *>(this_tf->view.get())->strides, s * dynamic_cast<StridedLayout *>(this_tf->view.get())->strides[dim]),
        this_tf->grad->data_);
