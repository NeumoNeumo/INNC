...
std::unique_ptr<TensorFrame> TensorFrame::slice(const std::vector<std::array<int, 3>>& slices) const {
    ...
    for (size_t i = 0; i < slices.size(); i++)
    {
      if (slices[i][0]+slices[i][2]>slices[i][1])
      {
        continue;
      }
      a.push_back((slices[i][1]-slices[i][0])/slices[i][2]);
      b.push_back(this->strides[i]*slices[i][2]);   
    }
    size_t c=this->offset;
    for (size_t i = 0; i < slices.size(); i++)
    {
      c+=slices[i][0]*this->strides[i];
    }
    std::unique_ptr<TensorFrame> d = std::make_unique<TensorFrame>(this->dtype, a, b, c);
    d->data_ = this->data_; 
    return d; 
}
