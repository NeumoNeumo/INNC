...
TEST(basic, initialization) {
  using INNC::Tensor;
  auto a = Tensor::zeros({1}, INNC::i16);
  ASSERT_EQ(a.to_string(), "[0]");
  a = Tensor::zeros({1}, INNC::i32);
  ASSERT_EQ(a.to_string(), "[0]");
  a = Tensor::zeros({1, 1}, INNC::f32);
  ASSERT_EQ(a.to_string(), "[[" + std::to_string(float(0)) + "]]");
  a = Tensor::zeros({1, 2, 3}, INNC::i8);
  ASSERT_EQ(a.to_string(), "[[[0, 0, 0], [0, 0, 0]]]");
  ASSERT_EQ(a.numel(), 6);
  a = Tensor::ones({1, 2, 3}, INNC::i32);
  ASSERT_EQ(a.to_string(), "[[[1, 1, 1], [1, 1, 1]]]");
  a = Tensor::ones({2, 1, 1}, INNC::f32);
  std::string f32_one_str = std::to_string(double(1));
  ASSERT_EQ(a.to_string(),
            "[[[" + f32_one_str + "]], [[" + f32_one_str + "]]]");
  a = Tensor::zeros({3, 2, 5}, INNC::i8);
  auto b = Tensor::zeros_like(a);
  ASSERT_EQ(a.to_string(), b.to_string());
  a = Tensor::ones({4, 5, 6}, INNC::i8);
  b = Tensor::ones_like(a);
  ASSERT_EQ(a.to_string(), b.to_string());
  a = Tensor::from_blob(data_i8_1, {2, 3}, INNC::i8);
  ASSERT_EQ(a.to_string(), output_i8_1);
}

TEST(basic, type) {
  using INNC::Tensor;
  auto a = Tensor::from_blob(data_i8_1, {2, 3}, INNC::i8);
  ASSERT_EQ(a.to_string(), output_i8_1);
  auto b = a.type(INNC::i64);
  ASSERT_EQ(b.to_string(), output_i8_1);
  a = Tensor::from_blob(data_i8_1 + 1, INNC::SizeVec{1}, INNC::i8);
  a = a.type(INNC::f64);
...
TEST(arithmetic, add) {
  using INNC::Tensor;
  auto a = Tensor::ones({2, 3}, INNC::i16);
  auto b = Tensor::ones({2, 3}, INNC::i32);
...
TEST(index, slice) {
  ...
  //  [6, 7, 8],
  //  [9,10,11]]
  auto a = Tensor::from_blob(data_i16_2, {4, 3}, INNC::i16);
  auto b = a[""];
...
TEST(index, transpose) {
  using INNC::Tensor;
  auto a = Tensor::from_blob(data_i16_2, {4, 3}, INNC::i16);
  auto b = Tensor::transpose(a, 0, 1);
  ASSERT_EQ(b.to_string(), "[[0, 3, 6, 9], [1, 4, 7, 10], [2, 5, 8, 11]]");
  a = Tensor::from_blob(data_i8_1, {2, 3}, INNC::i8);
  b = a.transpose(a, 0, 1);
  ASSERT_EQ(b.to_string(), "[[1, 7], [-3, -9], [-5, 11]]");
  a = Tensor::from_blob(data_i16_2, {2, 3, 2}, INNC::i16);
  b = Tensor::transpose(a, 0, 2);
  ASSERT_EQ(b.to_string(),
            "[[[0, 6], [2, 8], [4, 10]], [[1, 7], [3, 9], [5, 11]]]");
  a = Tensor::from_blob(data_i8_1, {3, 2}, INNC::i8).type(INNC::f64);
  a = a.transpose(0, 1);
  double f64_arr[6] = {1, -5, -9, -3, 7, 11};
  b = Tensor::from_blob(f64_arr, {2, 3}, INNC::f64);
  ASSERT_EQ(a.to_string(), b.to_string());
...
TEST(arithmetic, mul) {
  using INNC::Tensor;
  auto a = Tensor::from_blob(data_i8_1, {2, 3}, INNC::i8);
  auto b = Tensor::from_blob(data_i16_1, {2, 3}, INNC::i16);
...
TEST(arithmetic, sum) {
  ...
  a = Tensor::from_blob(data_i16_1, {3, 2}, INNC::i16);
  ASSERT_EQ(a.sum().to_string(), "[6]");
}

TEST(autograd, add) {
  using INNC::Tensor;
  auto a = Tensor::ones({2, 3}, INNC::f64);
  auto b = Tensor::ones({2, 3}, INNC::f32);
  ...
  a = Tensor::ones({1}, INNC::f32);
  b = Tensor::ones({1}, INNC::f64);
  a.requires_grad(true);
  b.requires_grad(true);
  c = a + a;
  d = c + c;
...
TEST(autograd, mul) {
  using INNC::Tensor;
  auto a = Tensor::from_blob(data_i8_1, {2, 3}, INNC::i8).type(INNC::f32);
  auto b = Tensor::from_blob(data_i8_1, {2, 3}, INNC::i8).type(INNC::f64);
  a.requires_grad(true);
  b.requires_grad(true);
  auto c = a * b;
  c.sum().backward();
  ...
  a.zero_grad();
  b = Tensor::from_blob(data_i8_1, {2, 3}, INNC::i8);
  (a * b).sum().backward();
...
TEST(autograd, transpose) {
  using INNC::Tensor;
  auto a = Tensor::from_blob(data_i16_1, {2, 3}, INNC::i16).type(INNC::f64);
